...
const pokemonList = [
  ...
  function compareTypes(guess, target) {
    if (guess.type === target.type) {
      return "Você acertou!";
    } else if (guess.type === "Elétrico" && target.type === "Planta") {
      return "O Pokémon que você escolheu é de tipagem mais fraca.";
    } else if (guess.type === "Fogo" && target.type === "Elétrico") {
      return "O Pokémon que você escolheu é de tipagem mais fraca.";
    } else if (guess.type === "Água" && target.type === "Fogo") {
      return "O Pokémon que você escolheu é de tipagem mais fraca.";
    } else {
  ...
  function playGuessThePokemon() {
    ...
    const guessedPokemon = pokemonList.find(pokemon => pokemon.name.toLowerCase() === guess.toLowerCase());
